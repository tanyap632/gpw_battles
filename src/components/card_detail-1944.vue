...
onMounted(async () => {
  try {
    const response = await fetch('https://98ec1f7320b02e63.mokky.dev/operation_1944')
...
const goToHome = () => {
  if (router && router.push) {
    router.push({ name: 'home', hash: '#operation1944'  })
  } else {
    console.error('Роутер не определен')
  }
}
</script>
<!-- <div class="max-w-520 mb-127 hidden"></div>
    <div class="max-w-522 mb-69 hidden"></div>
     <div class="mb-41 hidden"></div>
      <div class="mb-98 hidden"></div> -->
