...
onMounted(async () => {
  try {
    const response = await fetch('https://98ec1f7320b02e63.mokky.dev/operation_1941')
...
const goToHome = () => {
  if (router && router.push) {
    router.push({ name: 'home', hash: '#operation1941' })
  } else {
    console.error('Роутер не определен')
  }
}
</script>
<!-- <div class="max-w-483 mb-98 hidden"></div>
